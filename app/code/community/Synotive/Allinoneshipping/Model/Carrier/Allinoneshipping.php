...
*/
class Synotive_Allinoneshipping_Model_Carrier_Allinoneshipping extends Mage_Shipping_Model_Carrier_Abstract
{
    protected $_code = 'synotiveallinoneshipping';
    ...
    public function collectRates(Mage_Shipping_Model_Rate_Request $request) 
    {
        ...
        $collection = Mage::getResourceModel('synotiveallinoneshipping/method_collection')
            ->addFieldToFilter('is_active', 1)
            ->addStoreFilter($request->getStoreId())
            ->addCustomerGroupFilter($this->getCustomerGroupId($request))
            ->setOrder('pos'); 
                            
        $rates = Mage::getModel('synotiveallinoneshipping/rate')->findBy($request, $collection); 
        ...
        foreach ($collection as $customMethod){
            ...
            if (isset($ratesNew[$customMethod->getId()]))
            {
                ...
                $final[$i]['method_title'] = Mage::helper('synotiveallinoneshipping')->__($customMethod->getName());        
    ...
    public function getAllowedMethods()
    {
        ...
        foreach ($collection as $method){
            $methodCode = 'synotiveallinoneshipping'.$method->getMethodId();
